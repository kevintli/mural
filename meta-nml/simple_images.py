...
for ep in range(num_epochs):
    ...
    """grid_vals_torch = torch.from_numpy(images_grid).float().cuda()
    feats = model.embedding(grid_vals_torch).cpu().detach().numpy()
    query_feat = feats[-1]
    d = np.dot(query_feat, feats.T)/(np.linalg.norm(query_feat)*np.linalg.norm(feats, axis=-1))
    imshow(d.reshape(16, 16), cmap='RdBu', clim=(-1, 1))
    plt.savefig('distance_weightings/dist_%d.png'%ep)"""
